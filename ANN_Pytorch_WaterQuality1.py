...
# Entraînement réalisé soit sur le GPU ou le CPU
...
# Fonction permettant de calculer la précision
def accuracy(predictions, label):
    label1 = torch.argmax(predictions, dim=1)
    return torch.mean((label1 == label).float())
...
for epoch in range(0, EPOCHS):
    for i, data in enumerate(train_dataloader):  # parcours d'un batch
        ...
        # Prédictions et calcul de l'erreur & de la précision
    ...
    # Enregistrement de chaque erreur & de chaque précision
    losses.append((epoch + 1, round(loss.item(), 3)))
    accuracies.append((epoch + 1, round(correct.item(), 3)))

    # Affichage
    print('Epoch [{}/{}]\n Accuracy: {:.3f} Loss: {:.3f}'.format(epoch + 1, EPOCHS, correct.item(), loss.item()))
print("Entraînement terminé !!")

# Enregistrement de l'historique de l'entraînement dans un fichier csv
with open('text_files/historyloss_waterquality.csv', 'w') as file:
    file_csv = csv.writer(file)
    file_csv.writerow(['Epoch', 'Loss'])
    for ligne in losses:
        file_csv.writerow(ligne)
print("\nEnregistrement de l'historique d'erreurs réalisé avec succès !!")

with open('text_files/historyaccuracy_waterquality.csv', 'w') as file1:
    file_csv1 = csv.writer(file1)
    file_csv1.writerow(['Epoch', 'Accuracy'])
    for ligne1 in accuracies:
        file_csv1.writerow(ligne1)
print("Enregistrement de l'historique de précisions réalisé avec succès !!")

# Traçage des courbes
columns = ['Epoch', 'Loss']
df2 = pd.read_csv('text_files/historyloss_waterquality.csv', usecols=columns, delimiter=',')
plt.plot(df2.Epoch, df2.Loss)
plt.title('Erreur du modèle', fontsize=20)
plt.show()

columns1 = ['Epoch', 'Accuracy']
df3 = pd.read_csv('text_files/historyaccuracy_waterquality.csv', usecols=columns1, delimiter=',')
plt.plot(df3.Epoch, df3.Accuracy)
plt.title('Précision du modèle', fontsize=20)
plt.show()
